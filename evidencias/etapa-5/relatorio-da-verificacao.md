...
| A02 | **Redirecionamento Externo (Open Redirect)** | Alerta vermelho (High) no painel do OWASP ZAP, indicando que um parâmetro de URL controlado pelo usuário é utilizado para determinar o destino do redirecionamento sem validação contra uma lista de destinos permitidos. | Um atacante pode construir um endereço que começa no domínio legítimo e termina em um site sob seu controle, aumentando a eficácia de campanhas de phishing e podendo vazar o token de sessão pelo cabeçalho `Referer`. | CWE-601 (Redirecionamento de URL para Site Não Confiável) e OWASP Top 10:2021 — A01: Broken Access Control. | Eliminar o redirecionamento baseado em URL recebida do cliente, substituindo-o por identificadores internos mapeados no servidor para uma lista de destinos permitidos. |
| A03 | **Session ID in URL Rewrite** | Alerta laranja (Medium, classificado como *Systemic*) no painel do OWASP ZAP, evidenciando a presença do identificador de sessão na *query string* das requisições, em vez de exclusivamente em cookie. | O identificador de sessão fica exposto em histórico de navegador, logs de servidor e proxies e no cabeçalho `Referer` enviado a terceiros, permitindo que um atacante reutilize a sessão de um usuário autenticado sem conhecer sua senha. | CWE-598 (Uso do Método GET com Strings de Query Contendo Dados Sensíveis) e OWASP Top 10:2021 — A07: Identification and Authentication Failures. | Transportar o identificador de sessão exclusivamente por cookie com os atributos `HttpOnly`, `Secure` e `SameSite`, rejeitando qualquer sessão recebida por parâmetro de URL. |
